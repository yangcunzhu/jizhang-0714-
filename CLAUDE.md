...
### 🔴 铁律 9(2026-07-19 增)：v4 是 SSOT,ADR 只能补充,开工前必查 v4
...
### 🔴 铁律 10(2026-07-19 增)：完成日期机制,文档对账
...
### 🔴 铁律 11(2026-07-19 增)：启动前,我要列要点清单,你照拍
**(用户问题倒逼出来的规则 — 我之前没主动列要点,你要我"自己脑补",导致漏细节)。**

- 每个新功能 / 新 Stage 开工前,**我必须**:
  1. 列出要看的要点清单(元素 / 字段 / 状态 / 边界)
  2. 你按清单拍(不漏)
  3. 我对照 v4 + ADR 写实施
- 不要「自己脑补」 — 你给图就只看着你给的图,不要假设有更多
- 不要「跳着做」 — 一步步对齐再写代码

### 🔴 铁律 12(2026-07-19 增)：每个 Stage 收尾,我要自审偷懒点
**(用户问题倒逼出来的规则 — 我之前只听你反馈,不主动找偷懒点)。**

- 每个 Stage 收尾(写完 commit 后),**我必须**自审:
  1. 对照 v4 §3(咔皮对标)找偷懒点
  2. 对照 ADR 决策找没做 / 没做透的地方
  3. 输出「自审报告」给你(修了什么 + 漏了什么)
- 不要等你反馈 — 主动找
- 跑偏检查:每 1-2 周 Read v4 + 当前代码 diff
