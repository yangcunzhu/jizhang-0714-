...
| 资金账户 💵🏦 | 当前余额 | 「余额 ¥xxx」 |
| 充值账户 🚌 | 当前余额 | 「余额 ¥xxx」 |
| 信用账户 💳(信用卡/花呗/京东白条/借呗/抖音放心贷/抖音月付/自定义)| **已用额度**(0 = 没用)| 「已用 ¥xxx / 额度 ¥yyy,可用 ¥(creditLimit - balanceCents)」 |
| 理财账户 📈 | 当前余额 | 「余额 ¥xxx」 |
| ⚠️ 借贷账户(loan 大类,D21 已加 subType=lendOut/borrowIn 字段占位)| **D22 决策:不维护借贷账户余额**,借贷通过 transaction(type=lend/borrow)维护,详 ADR-0028 §2.1 | 借贷无账户余额概念,资金方走 transaction 联动 |
...
| 转账(D21 扩)| transfer | `fromAccount.balanceCents -= amountCents` + `toAccount.balanceCents += amountCents` |
| 还款(D19)| repayment | `储蓄.balanceCents -= amountCents` + `信用卡.balanceCents -= amountCents` |
| 借出(D22 扩)| lend | `fromAccount.balanceCents -= amountCents`(资金方扣款,**不维护借贷账户余额**)|
| 借入(D22 扩)| borrow | `toAccount.balanceCents += amountCents`(资金方入款,**不维护借贷账户余额**)|
...
- **Stage 5(净资产 & 仪表盘)**:基于 `includeInNetWorth` + `balanceCents` 计算净资产(**借贷不计入**,详 ADR-0026 §6/§8 D22 修订 + ADR-0028 §2.1)
