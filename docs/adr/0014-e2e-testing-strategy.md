...
**采用 `integration_test` 包(Flutter SDK 自带),建立真 Flutter engine 集成测试**
...
**包选择**:
- ✅ `integration_test`(Flutter SDK 自带,无版本风险,跟 Flutter SDK 自动对齐)
...
| 5 | 0 成本扩展 | `integration_test` 是 SDK 自带,无需额外依赖 + 许可证 |
